...
// x coordinates are scaled by xscale and y coordinates scaled by yscale
{
	...
	for (int i = 0; i<count; ++i) {
		int xCoord = xy.x+int(r*xscale);
		int yCoord = xy.y-int(f(r)*yscale);
		if(xCoord >= -32768 && yCoord >= -32768 && xCoord <= 32767 && yCoord <= 32767) ///clip with MAX_SHORT
			add(Point(xCoord,yCoord));
		r += dist;
	}
}
...
{
	__attribute__ ((unused)) static int x = init_suffix_map();
